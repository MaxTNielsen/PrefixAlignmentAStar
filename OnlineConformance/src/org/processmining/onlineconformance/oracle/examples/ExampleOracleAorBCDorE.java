...
public class ExampleOracleAorBCDorE implements HistoryAwareOracle<ExampleState, TestExample.LM, TestExample.MM> {

	public ExampleOracleAorBCDorE() {
	...
	public List<SimpleHistoryAwareMovementSequence<ExampleState, TestExample.LM, TestExample.MM>> getSyncronousMoveSequences(
		...
		} else if (currentState.equals(new ExampleState("P2P6"))) {
			if (label == LM.A) {
				//not possible
			} else if (label == LM.B) {
				//not possible
		...
		} else if (currentState.equals(new ExampleState("P3P5"))) {
			if (label == LM.A) {
				//not possible
			} else if (label == LM.B) {
				//not possible
		...
		} else if (currentState.equals(new ExampleState("P3P6"))) {
			if (label == LM.A) {
				//not possible
			} else if (label == LM.B) {
				//not possible
	...
	private ExampleState execute(ExampleState currentState, List<MoveImpl<TestExample.LM, TestExample.MM>> moves) {
		if (moves.isEmpty()) {
			...
			if (m.getTransition() == MM.A) {
				...
				return new ExampleState("P2P6");
			} else if (m.getTransition() == MM.C) {
				if (currentState.equals(new ExampleState("P2P6"))) {
					return new ExampleState("P3P6");
				} else if (currentState.equals(new ExampleState("P2P5"))) {
					return new ExampleState("P3P5");
				} else if (currentState.equals(new ExampleState("P1"))) {
					return new ExampleState("P3P6");
				}
